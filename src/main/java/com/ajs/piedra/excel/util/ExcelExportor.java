package com.ajs.piedra.excel.util;

import com.ajs.piedra.excel.Constants;
import com.ajs.piedra.excel.annotation.ExcelExport;
import com.ajs.piedra.excel.bean.ExcelHeader;
...
public class ExcelExportor<T> {
    ...
    /** 导出的内容的文本水平对齐方式*/
    private Short contCellTextAlign = HSSFCellStyle.ALIGN_LEFT;
    /** 导出的内容的文本垂直对齐方式*/
    ...
    public Map<String,String> exportExcelMulSheet(Map<String , List<T> > mapList, OutputStream out) {
        ...
        for(Map.Entry<String, List<T>> entry:mapList.entrySet()){
            ...
            List<T> dataRows = dataset.subList(0, dataset.size());
            exportRowDatas(dataRows, workbook, sheet, maxRows);
        }
